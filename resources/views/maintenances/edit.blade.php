        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 my-4">
            <div class="border border-gray-200 dark:border-gray-600 rounded-xl p-6 shadow-2xl">
                <div class="font-black text-center border-b-2 border-gray-200 dark:border-gray-600 py-2">
                    Editando {{ $maintenance->name }}
                </div>
                <form class="w-full max-w-lg p-2" action='/maintenances/{{ $maintenance->id}}' method="post" id="formMaintenance">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="user_id" id="user_id" value="{{ $maintenance->user_id }}" required>
                        ...
                        <div class="relative w-full mb-6 md:mb-0">
                            ...
                            <select class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 dark:bg-gray-700 dark:border-gray-900 dark:text-gray-200 dark:focus:bg-gray-600 dark:focus:text-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" name="type" id="type" required>
                                ...
                                <optgroup label="Hardware">
                                    <option value="hardware/remoção" {{ $maintenance->subtype == 'remoção' ? 'selected' : ''}}>Remoção</option>
                                    <option value="hardware/adição" {{ $maintenance->subtype == 'adição' ? 'selected' : ''}}>Adição</option>
                                    <option value="hardware/substituição" {{ $maintenance->subtype == 'substituição' ? 'selected' : ''}}>Substituição</option>
                                </optgroup>
                                <optgroup label="Software">
                                    <option value="software/instalação" {{ $maintenance->subtype == 'instalação' ? 'selected' : ''}}>Instalação</option>
                                    <option value="software/atualização" {{ $maintenance->subtype == 'atualização' ? 'selected' : ''}}>Atualização</option>
                                    <option value="software/desinstalação" {{ $maintenance->subtype == 'desinstalação' ? 'selected' : ''}}>Desinstalação</option>
                                    <option value="software/configuração" {{ $maintenance->subtype == 'configuração' ? 'selected' : ''}}>Configuração</option>
                                    <option value="software/imageação" {{ $maintenance->subtype == 'imageação' ? 'selected' : ''}}>Imageação</option>
                                </optgroup>
                                <optgroup label="Outro">
                                    <option value="outro/outro" {{ $maintenance->subtype == 'outro' ? 'selected' : ''}}>Outro</option>
                                </optgroup>
                            </select>
